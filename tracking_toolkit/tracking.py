...
def init_handles():
    ...
    global action_handles, skeleton_handles
    action_handles = {
...
def _get_input(ovr_context: OVRContext):
    ...
    # Finger curls: prioritize skeletal data for Knuckles, fallback to analog channels, then trigger/grip estimation.
    l_skeletal = _get_skeletal_curls(vr_ipt, skeleton_handles.get("left"))
    r_skeletal = _get_skeletal_curls(vr_ipt, skeleton_handles.get("right"))
...
def _get_skeletal_curls(vr_ipt, skeleton_action_handle):
    if not skeleton_action_handle:
        return None

    get_skeletal_action_data = getattr(vr_ipt, "getSkeletalActionData", None)
    get_skeletal_bone_data = getattr(vr_ipt, "getSkeletalBoneData", None)
    if not get_skeletal_action_data or not get_skeletal_bone_data:
        return None

    try:
        action_data = get_skeletal_action_data(skeleton_action_handle)
        if not getattr(action_data, "bActive", False):
            return None

        # Prefer model space + with controller range when available.
        ...
        bones = get_skeletal_bone_data(skeleton_action_handle, transform_space, motion_range)
    except Exception:
        return None

    if not bones or len(bones) < 26:
        return None

    def _curl(start_i: int, mid_i: int, tip_i: int) -> float:
        try:
            a = Vector(bones[start_i].position.v[:3])
            b = Vector(bones[mid_i].position.v[:3])
            c = Vector(bones[tip_i].position.v[:3])
            v1 = (a - b)
            v2 = (c - b)
            if v1.length == 0 or v2.length == 0:
                return 0.0
            angle = v1.angle(v2)
            return max(0.0, min(1.0, angle / 3.14159265))
        except Exception:
            return 0.0

    # OpenVR hand skeleton indices
    return {
        "thumb": _curl(2, 4, 5),
        "index": _curl(6, 8, 10),
        "middle": _curl(11, 13, 15),
        "ring": _curl(16, 18, 20),
        "pinky": _curl(21, 23, 25),
    }
