...
def init_handles():
    ...
    action_handles = {
        ...
        # Skeletal actions (Knuckles finger tracking)
...
def _get_input(ovr_context: OVRContext):
    ...
    def _get_skeletal_curls(action_key: str):
        action = action_handles.get(action_key)
        if action is None:
            return None

        # Prefer summary API (direct finger curls) when available.
        try:
            if hasattr(vr_ipt, "getSkeletalSummaryData"):
                # 0 is usually "FromAnimation"/default summary type in OpenVR bindings.
                summary = vr_ipt.getSkeletalSummaryData(action, 0)
                curls = getattr(summary, "flFingerCurl", None)
                if curls is not None and len(curls) >= 5:
                    return [float(curls[i]) for i in range(5)]
        except Exception:
            pass

        # Fallback: try to read from skeletal action data if summary isn't available.
        try:
            if hasattr(vr_ipt, "getSkeletalActionData"):
                skel_data = vr_ipt.getSkeletalActionData(action)
                if getattr(skel_data, "bActive", False) is False:
                    return None
        except Exception:
            pass
    ...
    # Finger curls: prefer skeletal summary (Knuckles), then per-finger analog channels, then trigger/grip fallback.
    l_skel_curls = _get_skeletal_curls("l_skeleton")
    r_skel_curls = _get_skeletal_curls("r_skeleton")

    if l_skel_curls:
        l_ipt.thumb_curl, l_ipt.index_curl, l_ipt.middle_curl, l_ipt.ring_curl, l_ipt.pinky_curl = l_skel_curls
    else:
    ...
    if r_skel_curls:
        r_ipt.thumb_curl, r_ipt.index_curl, r_ipt.middle_curl, r_ipt.ring_curl, r_ipt.pinky_curl = r_skel_curls
    else:
